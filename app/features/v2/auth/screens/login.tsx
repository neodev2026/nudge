 ...
 * GET/POST /login
 *
 * v2 login page — email+password + Google OAuth + Discord OAuth.
 * On success redirects to ?next= (default: /products).
 */
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const { data: valid, success, error } = loginSchema.safeParse(
  ...
  const { error: signInError } = await client.auth.signInWithPassword(valid);
      ...
      <div className="w-full max-w-sm">
        ...
        <Form method="post" className="space-y-4">
          <div>
            <label
              htmlFor="email"
              className="mb-1.5 block text-xs font-bold text-[#6b7a99]"
            >
              이메일
            </label>
            <input
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between">
              <label
                htmlFor="password"
                className="text-xs font-bold text-[#6b7a99]"
              >
                비밀번호
              </label>
              <Link
                to="/auth/forgot-password/reset"
                className="text-xs text-[#6b7a99] underline hover:text-[#1a2744]"
              >
                비밀번호 찾기
              </Link>
            </div>
        ...
        <p className="mt-6 text-center text-sm text-[#6b7a99]">
          계정이 없으신가요?{" "}
          <Link
            to={`/join${next !== "/products" ? `?next=${encodeURIComponent(next)}` : ""}`}
